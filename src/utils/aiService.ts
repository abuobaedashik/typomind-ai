...
export const generateMedicalResponse = async (
  ...
  try {
    ...
    // Instructions array (now strings)
    const instructions = [
    ...
    // Shuffle instructions (Fisher-Yates)
...
export const detectLanguage = async (text: string): Promise<string> => {
  ...
  const totalLetters = Object.values(scores).reduce(
    (sum, count) => sum + count,
    0
  );
  const englishRatio =
    totalLetters > 0 ? scores.English / totalLetters : 0;
